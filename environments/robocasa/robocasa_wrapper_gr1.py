...
class RoboCasaWrapperGR1:
    """
    Wrapper for GR1FixedLowerBody dual-arm robot in RoboCasa environments.

    This wrapper is designed specifically for the GR1FixedLowerBody humanoid robot
    which has two arms with dexterous grippers and uses a WHOLE_BODY_IK controller.

    Action Space (24 dimensions):
    - 6 dimensions: right arm end effector
    - 6 dimensions: left arm end effector
    - 6 dimensions: right gripper Fourier hands (5 fingers + 2 thumb actions)
    - 6 dimensions: left gripper Fourier hands (5 fingers + 2 thumb actions)

    Observations:
    - Three cameras: agent view center, right hand eye-in-hand, left hand eye-in-hand
    - Dual-arm state: left and right end-effector positions, quaternions
    - 14 joint positions (joint_pos_cos/sin)
    - 12D gripper state per hand (left_gripper_qpos, right_gripper_qpos)
    """
    ...
    def __init__(
        ...
        self.env = env
        if keys is None:
            # Default keys for GR1ArmsOnly with three cameras
            keys = [
                "agentview_image",
